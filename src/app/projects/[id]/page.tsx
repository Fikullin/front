...
export default function ProjectView() {
  ...
  const handleAddTask = async (newTask: Task) => {
    // After adding a new task, refetch tasks to ensure UI is up to date and consistent
    const id = Array.isArray(params.id) ? params.id[0] : (params.id as string);
    if (id) {
      try {
        // Add a small delay before refetching to avoid stale cache issues
        setTimeout(async () => {
          const tasksData = await getTasksByProjectId(parseInt(id));
          setTasks(tasksData || []);
        }, 500);
      ...
      // Fallback: just add the new task to the state
